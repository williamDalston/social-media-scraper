...
def generate_report(db_path="social_media.db"):
    ...
    try:
        ...
        platform_counts = (
            session.query(
                DimAccount.platform, func.count(DimAccount.account_key).label("count")
            )
            .group_by(DimAccount.platform)
            .all()
        )

        print("\nAccounts by Platform:")
        for platform, count in sorted(
            platform_counts, key=lambda x: x[1], reverse=True
        ):
        ...
        org_counts = (
            ...
            .group_by(DimAccount.organization)
            .all()
        )
        ...
        if latest_date:
            ...
            snapshot_counts = (
                ...
                .group_by(FactFollowersSnapshot.snapshot_date)
        ...
        if latest_date:
            ...
            accounts_with_data = (
                session.query(
                    DimAccount.platform,
                    func.count(distinct(FactFollowersSnapshot.account_key)).label(
                        "count"
                    ),
                )
                .join(
            ...
            platform_data_map = {
                platform: count for platform, count in accounts_with_data
            }

            print("Accounts with Data (Latest Snapshot):")
            for platform, count in sorted(platform_data_map.items()):
                total_for_platform = next(
                    (c for p, c in platform_counts if p == platform), 0
                )
                percentage = (
                    (count / total_for_platform * 100) if total_for_platform > 0 else 0
                )
                status = (
                    "✓" if count == total_for_platform else "⚠" if count > 0 else "✗"
                )
                print(
                    f"  {status} {platform:20s}: {count:4d}/{total_for_platform:4d} ({percentage:5.1f}%)"
                )
            ...
            for platform in sorted(all_platforms):
                ...
                if missing > 0:
                    ...
                    accounts_without = (
                        ...
                        .filter(DimAccount.platform == platform)
                        .filter(
                    ...
                    for account in accounts_without:
                        print(
                            f"    - {account.organization[:40]:40s} ({account.handle})"
                        )
                    if missing > 10:
        ...
        if latest_date:
            ...
            metrics = (
                session.query(
                    func.avg(FactFollowersSnapshot.followers_count).label(
                        "avg_followers"
                    ),
                    func.max(FactFollowersSnapshot.followers_count).label(
                        "max_followers"
                    ),
                    func.min(FactFollowersSnapshot.followers_count).label(
                        "min_followers"
                    ),
                    func.sum(FactFollowersSnapshot.followers_count).label(
                        "total_followers"
                    ),
                    func.avg(FactFollowersSnapshot.engagements_total).label(
                        "avg_engagement"
                    ),
                    func.count(FactFollowersSnapshot.snapshot_key).label(
                        "total_snapshots"
                    ),
                )
                .filter(FactFollowersSnapshot.snapshot_date == latest_date)
            ...
            if metrics and metrics.total_snapshots:
                ...
                print(
                    f"Total Followers (All Platforms): {metrics.total_followers:,.0f}"
                )
        ...
        if latest_date:
            ...
            print(
                f"{'Platform':<20} {'Snapshots':<12} {'Total Followers':<18} {'Avg Followers':<15} {'Max Followers':<15}"
            )
            print("-" * 80)

            for platform, snapshots, total_fol, avg_fol, max_fol in sorted(
                platform_metrics, key=lambda x: x[2] or 0, reverse=True
            ):
                total_fol = total_fol or 0
                avg_fol = avg_fol or 0
                max_fol = max_fol or 0
                print(
                    f"{platform:<20} {snapshots:<12} {total_fol:>15,.0f} {avg_fol:>13,.0f} {max_fol:>13,.0f}"
                )
        ...
        if not latest_date:
            ...
            if missing_count > 0:
                print(
                    f"  ⚠ {missing_count} accounts are missing data. Consider re-running the scraper."
                )
            else:
            ...
            if days_old > 1:
                print(
                    f"  ⚠ Data is {days_old} days old. Consider running a fresh scrape."
                )
            else:
                print("  ✓ Data is fresh!")
