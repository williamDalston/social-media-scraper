...
def detect_account_takeover_risk(user_id: int, new_ip: str, new_user_agent: str = None) -> dict:
    """
    ...
    try:
        ...
        if recent_logins:
            previous_ips = set([log.ip_address for log in recent_logins if log.ip_address])
        ...
        if recent_logins and new_user_agent:
            previous_user_agents = [log.user_agent for log in recent_logins if log.user_agent]
