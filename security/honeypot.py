...
def honeypot_wp_admin():
    ...
    log_honeypot_access("wp-admin", {"method": request.method, "path": request.path})
    return jsonify({"error": "Not found"}), 404
...
def honeypot_env():
    ...
    log_honeypot_access(".env", {"method": request.method, "path": request.path})
    return jsonify({"error": "Not found"}), 404
...
def honeypot_config():
    ...
    log_honeypot_access("config.php", {"method": request.method, "path": request.path})
    return jsonify({"error": "Not found"}), 404
