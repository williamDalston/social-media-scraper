...
def check_security_policies():
    ...
    if SECURITY_POLICIES["validate_request_size"]:
        ...
        if content_length and content_length > MAX_REQUEST_SIZE:
            return (
                True,
                f"Request size exceeds maximum allowed size: {MAX_REQUEST_SIZE} bytes",
            )
        ...
        if request.is_json:
            try:
                ...
                if json_size > MAX_JSON_SIZE:
                    return (
                        True,
                        f"JSON payload size exceeds maximum: {MAX_JSON_SIZE} bytes",
                    )
            except Exception:
                pass
            ...
            if isinstance(value, str):
                for pattern in ATTACK_PATTERNS:
                    if re.search(pattern, value, re.IGNORECASE):
                        return (
                            True,
                            f"Attack pattern detected in parameter {key}: {pattern}",
                        )
                ...
                if isinstance(value, str):
                    for pattern in ATTACK_PATTERNS:
                        if re.search(pattern, value, re.IGNORECASE):
                            return (
                                True,
                                f"Attack pattern detected in form field {key}: {pattern}",
                            )
