        ...
        try:
            ...
            for log in old_logs:
                ...
                log.details = json.dumps(
                    {"anonymized": True, "original_date": log.timestamp.isoformat()}
                )
                deleted_count += 1
    ...
    def get_compliance_report(self) -> Dict:
        ...
        try:
            ...
            recent_logs = (
                ...
                .filter(AuditLog.timestamp >= datetime.utcnow() - timedelta(days=30))
                .count()
            )

            # Data retention status
            oldest_log = (
                session.query(AuditLog).order_by(AuditLog.timestamp.asc()).first()
            )
