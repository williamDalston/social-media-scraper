...
class LinkedInScraper(BasePlatformScraper):
    ...
    def __init__(self, max_sleep_seconds: Optional[float] = None):
        super().__init__("linkedin", max_sleep_seconds=max_sleep_seconds)
        ...
        try:
            ...
            if response.status_code == 403:
                raise PrivateAccountError(
                    f"LinkedIn page access denied: {normalized_url}"
                )

            if response.status_code != 200:
                raise ScraperError(
                    f"Failed to fetch LinkedIn page: {response.status_code}"
                )
            ...
            if BROWSER_AVAILABLE:
                logger.info(
                    f"Static HTML returned no data for LinkedIn page. Trying browser automation..."
                )
                try:
                    html = scrape_with_browser(
                        normalized_url, wait_time=12, driver_type="selenium"
                    )
                    if html:
                        ...
                        if followers > 0:
                            logger.info(
                                f"Successfully extracted data using browser automation for LinkedIn page"
                            )
                            return {
                                "followers_count": followers,
                                "following_count": 0,
            ...
            logger.warning(
                f"Could not extract data from LinkedIn page. LinkedIn may be blocking scrapers or page structure changed."
            )
            return {
                "followers_count": 0,
                "following_count": 0,
