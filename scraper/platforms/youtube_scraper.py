...
class YouTubeScraper(BasePlatformScraper):
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("YouTube API key not found. Will attempt web scraping fallback.")
    ...
    def _scrape_via_web(self, account_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                raise ScraperError(f"Failed to fetch YouTube page: {response.status_code}")
            ...
            meta_tags = soup.find_all('meta', {'property': re.compile(r'subscriber|subscriberCount', re.I)})
