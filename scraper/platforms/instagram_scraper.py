    ...
    def scrape_account(self, account_url: str, handle: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 404:
                raise AccountNotFoundError(f"Instagram account not found: {normalized_url}")
            
            if response.status_code == 403:
                raise PrivateAccountError(f"Instagram account is private: {normalized_url}")
            
            if response.status_code != 200:
                raise ScraperError(f"Failed to fetch Instagram page: {response.status_code}")
                ...
                try:
                    ...
                    if isinstance(data, dict):
                        if 'mainEntity' in data:
                            ...
                            if isinstance(entity, dict) and 'interactionStatistic' in entity:
                                for stat in entity['interactionStatistic']:
                                    if isinstance(stat, dict):
                                        ...
                                        if 'FollowAction' in interaction_type or 'followers' in interaction_type.lower():
            ...
            if followers == 0 or following == 0 or posts == 0:
                ...
                if posts == 0:
                    match = re.search(r'([\d.]+[KMBkmb]?)\s*posts?', text, re.IGNORECASE)
                    if match:
