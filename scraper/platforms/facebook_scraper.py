...
class FacebookScraper(BasePlatformScraper):
    ...
    def __init__(self):
        ...
        if not self.access_token:
            logger.warning("Facebook access token not found. Will use web scraping fallback.")
    ...
    def _scrape_via_api(self, page_id: str) -> Dict[str, Any]:
        ...
        if response.status_code == 401 or response.status_code == 403:
            raise AuthenticationError(f"Facebook API authentication failed: {response.status_code}")
    ...
    def _scrape_via_web(self, account_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                raise ScraperError(f"Failed to fetch Facebook page: {response.status_code}")
            ...
            if followers == 0:
                text = soup.get_text()
                match = re.search(r'([\d.]+[KMBkmb]?)\s*(?:followers?|people\s+like\s+this)', text, re.IGNORECASE)
                if match:
            ...
            if BROWSER_AVAILABLE:
                logger.info(f"Static HTML returned no data for Facebook page. Trying browser automation...")
                try:
                    html = scrape_with_browser(account_url, wait_time=8, driver_type='selenium')
                    if html:
                        ...
                        if followers > 0:
                            logger.info(f"Successfully extracted data using browser automation for Facebook page")
                            return {
                                'followers_count': followers,
                                'following_count': 0,
                                'posts_count': 0,
    ...
    def scrape_account(self, account_url: str, handle: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.access_token and page_id:
            try:
                ...
                logger.warning(f"API scraping failed, falling back to web scraping: {e}")
