...
class HistoricalCorrelator:
    ...
    def add_snapshot(self, account_key: int, snapshot: Dict[str, Any]):
        ...
        self._historical_data[account_key] = [
            s for s in self._historical_data[account_key]
            if datetime.fromisoformat(s.get('snapshot_date', '2000-01-01')).date() >= cutoff_date
        ]
    ...
    def get_historical_summary(self, account_key: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'followers_range': {
                ...
                'average': sum(s.get('followers_count', 0) or 0 for s in history) / len(history),
            },
        }
