...
def calculate_snapshot_metrics(
    ...
    if snapshot.followers_count and snapshot.followers_count > 0:
        if snapshot.engagements_total:
            snapshot.engagement_rate = round(
                (snapshot.engagements_total / snapshot.followers_count) * 100,
                4
            )
        else:
    ...
    try:
        ...
        previous_snapshot = session.query(FactFollowersSnapshot).filter(
        ...
        if previous_snapshot and previous_snapshot.followers_count and previous_snapshot.followers_count > 0:
    ...
    if account.platform.lower() == 'youtube':
        ...
        total_video_views = scraped_data.get('total_video_views') or scraped_data.get('view_count') or scraped_data.get('video_views', 0)
        snapshot.total_video_views = int(total_video_views) if total_video_views else None
        
        # Average views per video
        video_count = snapshot.videos_count or snapshot.posts_count or scraped_data.get('video_count', 0) or 0
        if video_count > 0 and snapshot.total_video_views:
            snapshot.average_views_per_video = round(
                snapshot.total_video_views / video_count,
                2
            )
        else:
