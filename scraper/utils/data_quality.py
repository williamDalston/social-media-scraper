...
class DataQualityScorer:
    ...
    def score_snapshot(self, snapshot_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        present_fields = sum(1 for field in required_fields if snapshot_data.get(field) is not None)
        completeness_score = present_fields / len(required_fields)
        
        if completeness_score < 1.0:
            issues.append(f"Missing {len(required_fields) - present_fields} required fields")
    ...
    def score_post(self, post_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        present_fields = sum(1 for field in required_fields if post_data.get(field) is not None)
        completeness_score = present_fields / len(required_fields)
    ...
    def generate_quality_report(self, snapshots: List[Dict[str, Any]], posts: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        """
        ...
        avg_snapshot_score = sum(s['score'] for s in snapshot_scores) / len(snapshot_scores)
        ...
        avg_post_score = sum(s['score'] for s in post_scores) / len(post_scores) if post_scores else 0.0
