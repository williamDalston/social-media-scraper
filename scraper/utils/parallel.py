...
class ScrapingMetrics:
    ...
    def get_summary(self) -> Dict:
        ...
        avg_time = (
            sum(self.account_times.values()) / len(self.account_times)
        ...
        return {
            ...
            "total_accounts": self.success_count
            + self.error_count
            + self.skipped_count,
            "average_time_per_account": round(avg_time, 3),
            "platform_counts": dict(self.platform_counts),
            "accounts_per_second": round(self.success_count / total_time, 2)
            if total_time > 0
            else 0,
        }
        ...
        if rate_limiter is not None:
            ...
            if lock:
                with lock:
                    ...
                    min_interval = (
                        0.5  # Minimum 0.5 seconds between scrapes per platform
                    )
            ...
            try:
                snapshot = FactFollowersSnapshot(
                    ...
                    subscribers_count=int(
                        data.get("subscribers_count", 0) or 0
                    ),  # For YouTube
...
def scrape_accounts_parallel(
    ...
    if prioritize_core:
        accounts = sorted(
            accounts, key=lambda a: (not (a.is_core_account or False), a.account_key)
        )
            ...
            try:
                ...
                if error:
                    ...
                    metrics.record_success(
                        account.account_key, account.platform, duration
                    )
                else:
