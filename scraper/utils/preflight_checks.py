...
def check_database_schema(db_path: str) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if missing_tables:
            return (
                False,
                f"Database schema not initialized. Missing tables: {', '.join(missing_tables)}. Run 'python scraper/main.py --init-db' first.",
            )
        ...
        return (
            False,
            f"Missing required module: {str(e)}. Install dependencies with 'pip install -r requirements.txt'",
        )
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if total_accounts == 0:
                return (
                    False,
                    "No accounts found in database. Please add accounts first using the bulk upload feature or populate from hhs_accounts.json.",
                    0,
                )
            ...
            if active_accounts == 0:
                return (
                    False,
                    f"Found {total_accounts} accounts but none are active. Please activate accounts in the database or check is_active flags.",
                    0,
                )
...
def check_scraper_imports() -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        return (
            False,
            f"Error importing scraper modules: {str(e)}. Check that all scraper files are present.",
        )
    except Exception as e:
...
def check_file_permissions() -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        try:
            ...
            return (
                False,
                f"Cannot write to current directory: {str(e)}. Check file permissions.",
            )
...
def run_preflight_checks(
    db_path: str = "social_media.db", include_network: bool = False
) -> Dict:
    """
    ...
    results["checks"].append(
        {
            ...
            if missing
            else "All required modules installed",
        }
    )
    if not passed:
        results["all_passed"] = False
        results["errors"].append(
            f"Missing required modules: {', '.join(missing)}. Install with: pip install {' '.join(missing)}"
        )
    else:
