...
class RateLimiter:
    ...
    def __init__(self, platform: str, requests: Optional[int] = None, window: Optional[int] = None):
        ...
        logger.info(f"Rate limiter initialized for {platform}: {requests} requests per {window} seconds")
    ...
    def wait_if_needed(self):
        ...
        with self.lock:
            ...
            if len(self.request_times) >= self.requests:
                ...
                wait_time = self.window - (now - oldest_request) + 0.1  # Add small buffer
                if wait_time > 0:
                    logger.info(f"Rate limit reached for {self.platform}. Waiting {wait_time:.2f} seconds...")
