...
class DimAccount(Base):
    ...
    )  # x, instagram, tiktok, youtube, truth_social
    ...
    account_category = Column(
        String
    )  # Platform-specific category (e.g., Government, Organization)
...
def _construct_sqlite_url(db_path: str) -> str:
    ...
    try:
        ...
        sqlite_url_obj = SQLAlchemyURL.create(drivername="sqlite", database=abs_path)
        ...
        try:
            ...
            raise ValueError(
                f"Failed to construct valid SQLite URL from '{db_path}'. " f"Error: {e}"
            ) from e
...
def init_db(db_path=None, enable_profiling: bool = False):
    ...
    if db_path:
        ...
        if url.endswith(".db") and not url.startswith(
    ...
    try:
        ...
        raise ValueError(
            f"Could not parse database URL '{url}': {url_error}"
        ) from url_error
    ...
    try:
        if is_sqlite:
            ...
            try:
                ...
                engine = create_engine(
                    url, poolclass=QueuePool, future=True, **pool_config
                )
            except ImportError:
                # PerformanceTuner not available - use default config
                engine = create_engine(url, poolclass=QueuePool, future=True)
        else:
            raise ValueError(
        ...
        raise ValueError(
            f"Failed to create database engine with URL '{url}': {e}"
        ) from e
