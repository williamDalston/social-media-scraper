...
def optimize_connection_pool(
    engine, pool_size=5, max_overflow=10, pool_timeout=30, pool_recycle=3600
):
    """
    ...
    logger.info(
        f"Connection pool optimized: size={pool_size}, overflow={max_overflow}, recycle={pool_recycle}s"
    )
...
def get_query_stats() -> dict:
    ...
    return {
        ...
        "p95_time": query_times_sorted[p95_idx]
        if p95_idx < len(query_times_sorted)
        else 0,
        "p99_time": query_times_sorted[p99_idx]
        if p99_idx < len(query_times_sorted)
        else 0,
