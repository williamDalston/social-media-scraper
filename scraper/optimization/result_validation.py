...
class ResultValidator:
    ...
    def validate_result(
        self, result: Dict, platform: str, account_handle: str
    ) -> Tuple[bool, List[str]]:
        """
        ...
        for field, value in result.items():
            if field in self.validation_rules:
                is_valid, error = self.validation_rules[field](
                    value, platform, account_handle
                )
                if not is_valid:
    ...
    def _validate_followers(
        self, value: int, platform: str, handle: str
    ) -> Tuple[bool, str]:
    ...
    def _validate_platform_specific(self, result: Dict, platform: str) -> List[str]:
        ...
        if platform == "X":
            ...
            if "listed_count" in result:
                if (
                    not isinstance(result["listed_count"], int)
                    or result["listed_count"] < 0
                ):
    ...
    def calculate_quality_score(
        ...
        # Check completeness
        required_fields = ["followers_count", "following_count", "posts_count"]
        completeness = sum(1 for field in required_fields if field in result) / len(
            required_fields
        )
