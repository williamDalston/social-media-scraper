...
class DataInsightsEngine:
    ...
    def analyze_performance(self) -> List[Insight]:
        ...
        try:
            from config.performance_monitoring import get_query_statistics, get_slow_queries
            ...
            for slo_name, slo_data in slo_status['slos'].items():
                if slo_data['status'] != 'met':
                    ...
                    insight = Insight(
                        ...
                        severity='critical' if slo_data['status'] == 'breached' else 'warning',
                        confidence=1.0,
    ...
    def analyze_usage_patterns(self) -> List[Insight]:
        ...
        if endpoint_usage:
            ...
            underutilized = [
                (endpoint, count) for endpoint, count in endpoint_usage.items()
    ...
    def analyze_trends(self) -> List[Insight]:
        ...
        try:
            ...
            for metric in metrics_to_analyze:
                ...
                if trend.get('direction') == 'decreasing' and trend.get('change_percent', 0) < -10:
                    self.insight_counter += 1
                    insight = Insight(
    ...
    def generate_all_insights(self) -> List[Insight]:
        ...
        return sorted(all_insights, key=lambda x: (
            {'critical': 0, 'warning': 1, 'info': 2}.get(x.severity, 3),
            -x.confidence
        ))
    
    def get_recent_insights(self, hours: int = 24, category: Optional[str] = None) -> List[Insight]:
        """Get recent insights."""
