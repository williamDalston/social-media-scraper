...
class AlertManager:
    ...
    def _is_duplicate(self, alert: Alert) -> bool:
        ...
        recent_alerts = [
            a
            for a in _alert_history[alert.source]
        ...
        for recent_alert in recent_alerts:
            if (
                ...
                and recent_alert.message == alert.message
                and recent_alert.severity == alert.severity
            ):
                return True
    ...
    def _send_email(self, alert: Alert):
        ...
        # For now, just log
        logger.info(
            f"EMAIL ALERT [{alert.severity.upper()}]: {alert.title} - {alert.message}"
        )
        ...
        for alerts in _alert_history.values():
            for alert in alerts:
                if alert.title == alert_title and not alert.acknowledged:
                    ...
                    logger.info(
                        f"Alert acknowledged: {alert_title} by {acknowledged_by}"
                    )
                    return True
        return False
