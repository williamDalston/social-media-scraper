...
class TrendAnalyzer:
    ...
    def calculate_trend(
        self, metric_name: str, window_minutes: Optional[int] = None
    ) -> Dict:
        """
    ...
    def forecast(
        self, metric_name: str, periods: int = 7, method: str = "linear"
    ) -> List[Dict]:
        """
    ...
    def compare_periods(
        ...
        period1_data = [
            d for d in data if period1_start <= d["timestamp"] <= period1_end
        ]
        period2_data = [
            d for d in data if period2_start <= d["timestamp"] <= period2_end
        ]
