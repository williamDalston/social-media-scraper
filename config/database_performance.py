...
class DatabasePerformanceMonitor:
    ...
    def __init__(self, max_history: int = 10000):
        ...
        self._query_patterns = defaultdict(
            lambda: {
                "count": 0,
                "total_time": 0.0,
                "min_time": float("inf"),
        ...
        self.slow_query_threshold = float(
            os.getenv("DB_SLOW_QUERY_THRESHOLD", "0.5")
        )  # 500ms
    ...
    def update_pool_stats(self, total: int, active: int, idle: int, overflow: int):
        """
    ...
    def get_overall_stats(self) -> Dict:
        ...
        with self._lock:
            if not self._queries:
                return {"total_queries": 0, "slow_queries_count": 0, "error_count": 0}
    ...
    def get_trends(self, hours: int = 24) -> Dict:
        ...
        with self._lock:
            ...
            recent_queries = [q for q in self._queries if q["timestamp"] >= cutoff]

            # Group by hour
            hourly_stats = defaultdict(
                lambda: {"count": 0, "total_time": 0.0, "errors": 0, "slow_queries": 0}
            )
...
def setup_query_monitoring(engine: Engine):
    ...
    def handle_error(context):
        ...
        try:
            statement = (
                context.statement if hasattr(context, "statement") else "unknown"
            )
            error = (
            ...
            monitor.record_query(
                query=statement, duration=duration, success=False, error=error
            )
        except Exception as e:
...
def record_db_query(
    query: str, duration: float, success: bool = True, error: Optional[str] = None
):
