...
class SLOTracker:
    ...
    def record_metric(self, slo_name: str, value: float, timestamp: Optional[datetime] = None):
        """
    ...
    def evaluate_slo(self, slo_name: str) -> Optional[SLOMeasurement]:
        ...
        if slo.metric == 'availability':
            ...
            successful = sum(1 for m in relevant_measurements if m['value'] < 500)  # HTTP < 500
            ...
            total = len(relevant_measurements)
            successful = sum(1 for m in relevant_measurements if m['value'] == 1)  # 1 = success
            value = (successful / total * 100) if total > 0 else 0
            status = 'met' if value >= slo.target else 'breached'
    ...
    def get_slo_compliance(self, slo_name: str, days: int = 30) -> Dict:
        ...
        return {
            ...
            'compliance_percent': (measurement.value / measurement.target * 100) if measurement.target > 0 else 0,
...
def record_slo_metric(slo_name: str, value: float, timestamp: Optional[datetime] = None):
    ...
    return {
        'slos': {
            name: {
                ...
                'target': measurement.target,
                'compliance': (measurement.value / measurement.target * 100) if measurement.target > 0 else 0,
