...
class PostIncidentReviewManager:
    ...
    def create_review(self, incident_id: str, scheduled_for: datetime,
                     participants: List[str]) -> PostIncidentReview:
        """
        ...
        logger.info(f"Created post-incident review: {review_id} for incident {incident_id}")
        return review
    ...
    def add_action_item(self, review_id: str, title: str, description: str,
                       assigned_to: str, priority: str = 'medium',
                       due_date: Optional[datetime] = None) -> ActionItem:
    ...
    def complete_review(self, review_id: str, root_cause: str, impact_assessment: str,
                       lessons_learned: List[str], improvements: List[str],
                       completed_by: str):
        """Complete a review."""
        if review_id not in self.reviews:
            return False
    ...
    def get_open_action_items(self, review_id: Optional[str] = None) -> List[ActionItem]:
        """Get open action items."""
        items = []
        
        reviews = [self.reviews[review_id]] if review_id and review_id in self.reviews else self.reviews.values()
        
        for review in reviews:
            items.extend([
                item for item in review.action_items
