...
class ReportGenerator:
    ...
    def generate_performance_report(self, period_days: int = 7) -> Report:
        ...
        if query_stats.get("p95_duration", 0) > 1.0:
            insights.append(
                f"P95 query duration is {query_stats['p95_duration']:.2f}s (target: <1s)"
            )
            recommendations.append("Consider optimizing slow queries or adding indexes")

        if resource_trends.get("memory", {}).get("rss_mb", 0) > 1000:
            insights.append(
                f"Memory usage is {resource_trends['memory']['rss_mb']:.1f}MB"
            )
            recommendations.append(
                "Monitor for memory leaks and consider optimizing memory usage"
            )
        ...
        if slo_status.get("slos"):
            for slo_name, slo_data in slo_status["slos"].items():
                if slo_data["status"] != "met":
                    ...
                    recommendations.append(
                        f"Investigate and fix issues affecting {slo_name}"
                    )
    ...
    def generate_trend_report(self, metric_name: str, period_days: int = 30) -> Report:
        ...
        if trend.get("direction") == "increasing":
            insights.append(
                f"{metric_name} is trending upward ({trend.get('change_percent', 0):.1f}% change)"
            )
        elif trend.get("direction") == "decreasing":
            insights.append(
                f"{metric_name} is trending downward ({trend.get('change_percent', 0):.1f}% change)"
            )
        ...
        report = Report(
            ...
            period_start=period_start,
            period_end=period_end,
            data={"metric_name": metric_name, "trend": trend, "forecast": forecast},
            insights=insights,
            recommendations=recommendations,
        )
    ...
    def generate_executive_report(self, period_days: int = 30) -> Report:
        ...
        if business_metrics.get("success_rate", 100) < 95:
            insights.append(
                f"Scraping success rate is {business_metrics.get('success_rate', 0):.1f}% (target: >95%)"
            )
            recommendations.append(
                "Investigate scraper failures and improve success rate"
            )

        # Usage insights
        if usage_summary.get("active_users", 0) > 0:
            insights.append(
                f"{usage_summary.get('active_users', 0)} active users in the last {period_days} days"
            )

        # SLO insights
        if slo_status.get("slos"):
            met_count = sum(
                1 for s in slo_status["slos"].values() if s["status"] == "met"
            )
            ...
            if met_count < total_count:
                recommendations.append(
                    "Address SLO violations to improve service quality"
                )
    ...
    def generate_usage_report(self, period_days: int = 7) -> Report:
        ...
        if endpoint_usage:
            ...
            insights.append(
                f"Most used endpoint: {top_endpoint[0]} ({top_endpoint[1]} calls)"
            )
        ...
        if feature_usage:
            ...
            insights.append(
                f"Most used feature: {top_feature[0]} ({top_feature[1]} uses)"
            )
