...
class MetricAggregator:
    ...
    def get_rollup(
        self, metric_name: str, labels: Optional[Dict] = None, periods: List[int] = None
    ) -> Dict[str, float]:
        """
    ...
    def get_trend(
        ...
        for bucket in sorted(buckets_data.keys()):
            ...
            timestamp = datetime.utcnow() - timedelta(
                minutes=(bucket + 0.5) * bucket_size
            )
            trend.append(
                {
...
def get_metric_rollup(
    metric_name: str, labels: Optional[Dict] = None
) -> Dict[str, float]:
...
def get_metric_trend(
    metric_name: str, labels: Optional[Dict] = None, window_minutes: int = 60
) -> List[Dict]:
