...
class ProductionPerformanceManager:
            ...
            for sla_name, sla in self._slas.items():
                ...
                if status == SLAStatus.VIOLATED:
                    violations.append({
                        ...
                        'status': status.value,
                        'details': details,
                        'severity': 'high' if details.get('p95_violated') else 'medium'
                    })
    ...
    def get_performance_summary(self) -> Dict:
        ...
        with self._lock:
            ...
            met_count = sum(
                1 for s in sla_statuses.values()
                if s['status'] == SLAStatus.MET.value
            )
            warning_count = sum(
                1 for s in sla_statuses.values()
            ...
            return {
                ...
                'compliance_rate': (met_count / total_slas * 100) if total_slas > 0 else 0,
...
def record_performance_metric(metric_name: str, value: float, timestamp: Optional[datetime] = None):
    """Record a performance metric."""
