...
if "register" in auth_bp.view_functions:
    limiter.limit("3 per hour", key_func=get_remote_address)(
        auth_bp.view_functions["register"]
    )
    ...
    try:
        ...
        try:
            results = (
                ...
                .options(joinedload(DimAccount))  # Eager load account data
                .all()
            )
        except Exception as query_error:
...
def api_history(platform, handle):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Error querying account: {query_error}",
        ...
        try:
            ...
            logger.error(
                f"Error querying history: {query_error}",
        ...
        try:
            data = {
                "dates": [
                    h.snapshot_date.isoformat() if h.snapshot_date else ""
                    for h in history
                ],
            ...
            logger.error(
                f"Error processing history data: {data_error}",
                extra={"account_key": account.account_key},
            )
            from api.errors import InternalServerError
...
def process_csv_data(csv_data, user=None):
    ...
    try:
        ...
        try:
            is_valid, error_message, validated_rows = validate_csv_file(
                csv_bytes, max_size_mb=10
            )
            ...
            for row_num, row in enumerate(validated_rows, start=1):
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error sanitizing org name for row {row_num}: {sanitize_error}"
                        )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error querying existing account: {query_error}",
                            extra={"row_num": row_num},
                        )
                    ...
                    if not existing:
                        try:
                            ...
                            logger.error(
                                f"Error creating account: {create_error}",
                                extra={"row_num": row_num},
                            )
        ...
        if session:
            try:
                ...
                logger.error(
                    f"Error closing session in process_csv_data: {close_error}"
                )
...
def get_job_status(job_id):
    ...
    try:
        ...
        return (
            jsonify({"error": f"Error getting job status: {str(e)}", "job_id": job_id}),
            500,
        )
...
def run_scraper():
    ...
    if mode != "real":
        return (
            jsonify(
                {"error": "Only real mode is supported. Simulations are disabled."}
            ),
            400,
        )
    ...
    try:
        ...
        logger.info(
            f"✓ All pre-flight checks passed. {preflight_results['account_count']} accounts ready to scrape."
        )
    except ImportError:
        ...
        logger.warning(
            f"Pre-flight checks encountered an error (continuing anyway): {str(e)}"
        )
        preflight_results = None
    ...
    try:
        ...
        return jsonify(
            {
                ...
                "status": "pending",
                "success": True,
                "account_count": preflight_results.get("account_count", 0)
                if preflight_results
                else None,
                "checks": preflight_results.get("checks", [])
                if preflight_results
                else None,
            }
        )
        ...
        return (
            jsonify({"error": f"Error running scraper: {str(e)}", "success": False}),
            500,
        )
...
def api_accounts():
    ...
    try:
        ...
        base_query = session.query(DimAccount).order_by(
            DimAccount.platform, DimAccount.handle
        )
...
def api_performance():
    ...
    try:
        ...
        # Add SLA status
        try:
            from config.production_performance import (
...
def api_performance_sla():
    ...
    try:
        ...
        return jsonify({"summary": get_performance_summary(), "slas": get_sla_status()})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
        ...
        return jsonify(
            {
                ...
                "critical_alerts": get_alert_history(
                    limit=20, severity=AlertSeverity.CRITICAL
                ),
            }
        )
    except Exception as e:
...
def admin_status():
    ...
    try:
        ...
        try:
            total_accounts = (
                session.query(func.count(DimAccount.account_key)).scalar() or 0
            )
            total_snapshots = (
                session.query(func.count(FactFollowersSnapshot.snapshot_key)).scalar()
                or 0
            )
...
def health():
    ...
    try:
        ...
        results = run_health_checks(
            db_path=db_path, include_optional=True, use_cache=True
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        check_thread = threading.Thread(
            target=check_critical_issues_periodically, daemon=True
        )
        check_thread.start()
