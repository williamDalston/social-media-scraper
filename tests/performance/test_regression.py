...
class PerformanceRegression:
    ...
    def check_regression(self, endpoint: str, current_time: float, 
...
def test_api_summary_regression(regression_tester, client):
    ...
    if result['status'] == 'regression':
        ...
        print(f"Performance OK: {avg_time*1000:.2f}ms (baseline: {result['baseline']*1000:.2f}ms)")
...
def test_api_history_regression(regression_tester, client):
    ...
    if result['status'] == 'regression':
        pytest.fail(f"Performance regression: {result['regression_percent']:.1f}% slower")
    elif result['status'] == 'no_baseline':
...
def test_cache_performance_regression(regression_tester):
    ...
    if result['status'] == 'regression':
        pytest.fail(f"Cache performance regression: {result['regression_percent']:.1f}% slower")
    elif result['status'] == 'no_baseline':
