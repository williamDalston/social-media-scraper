...
class LoadTest:
    ...
    def run_concurrent_requests(
        self, client_func, num_threads: int = 10, requests_per_thread: int = 10
    ) -> Dict:
        """
        ...
        results = {"success": 0, "errors": 0, "times": [], "status_codes": {}}
        ...
        def make_requests():
            ...
            for _ in range(requests_per_thread):
                ...
                try:
                    ...
                    elapsed = time.time() - req_start
                    thread_results.append(
                        {"success": False, "error": str(e), "time": elapsed}
                    )
...
def test_concurrent_summary_requests(load_test, client):
    ...
    result = load_test.run_concurrent_requests(
        make_request, num_threads=20, requests_per_thread=5
    )

    assert (
        result["success_rate"] >= 95
    ), f"Success rate {result['success_rate']}% below 95%"
...
def test_concurrent_history_requests(load_test, client):
    ...
    result = load_test.run_concurrent_requests(
        make_request, num_threads=10, requests_per_thread=5
    )
...
def test_sustained_load(load_test, client):
    ...
    while time.time() - start < 30:
        result = load_test.run_concurrent_requests(
            make_request, num_threads=10, requests_per_thread=1
        )
