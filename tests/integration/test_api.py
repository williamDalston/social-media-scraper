...
class TestSummaryAPI:
    ...
    def test_summary_returns_latest_metrics(
        self, client, app, db_session, sample_account
    ):
    ...
    def test_summary_returns_multiple_accounts(
        self, client, app, db_session, multiple_accounts
    ):
...
class TestHistoryAPI:
    ...
    def test_history_returns_account_history(
        self, client, app, db_session, sample_account
    ):
    ...
    def test_history_returns_empty_when_no_snapshots(
        self, client, app, db_session, sample_account
    ):
...
class TestDownloadAPI:
    ...
    def test_download_returns_csv(self, client, app, db_session, sample_account):
        ...
        assert "hhs_social_media_data.csv" in response.headers.get(
            "Content-Disposition", ""
        )
...
class TestRunScraperAPI:
    ...
    def test_run_scraper_with_simulated_mode(
        self, client, app, db_session, sample_account
    ):
        ...
        snapshot = (
            ...
            .filter_by(account_key=sample_account.account_key, snapshot_date=today)
            .first()
        )
    ...
    def test_run_scraper_with_real_mode(self, client, app, db_session, sample_account):
        ...
        response = client.post(
            "/api/run-scraper", json={"mode": "real"}, content_type="application/json"
        )
        # May succeed or fail depending on network, but should not crash
        assert response.status_code in [200, 500]

    def test_run_scraper_defaults_to_simulated(
        self, client, app, db_session, sample_account
    ):
        """Test that scraper defaults to simulated mode."""
        response = client.post(
            "/api/run-scraper", json={}, content_type="application/json"
        )
        assert response.status_code == 200
