...
class TestAccountCreation:
    ...
    def test_query_account_by_platform_and_handle(self, db_session, sample_account):
        ...
        account = db_session.query(DimAccount).filter_by(
            platform='X',
            handle='test_handle'
        ).first()
...
class TestSnapshotCreation:
    ...
    def test_query_latest_snapshot(self, db_session, sample_account):
        ...
        latest = db_session.query(FactFollowersSnapshot).filter_by(
            account_key=sample_account.account_key
...
class TestJoins:
    ...
    def test_join_account_and_snapshot(self, db_session, sample_account):
        ...
        # Query with join
        results = db_session.query(DimAccount, FactFollowersSnapshot).join(
            FactFollowersSnapshot
        ).filter(
            FactFollowersSnapshot.snapshot_date == date.today()
        ).all()
    ...
    def test_join_multiple_accounts(self, db_session, multiple_accounts):
        ...
        results = db_session.query(DimAccount, FactFollowersSnapshot).join(
            ...
            FactFollowersSnapshot.snapshot_date == today
        ).all()
...
class TestTransactions:
    ...
    def test_transaction_rollback_on_error(self, db_session):
        ...
        snapshot_check = db_session.query(FactFollowersSnapshot).filter_by(
            account_key=99999
        ).first()
