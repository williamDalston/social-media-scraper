...
class TestBackupProcedures:
    ...
    def test_database_backup_creation(self, test_db_path):
        ...
        backup_account = backup_session.query(DimAccount).filter_by(handle='backup_test').first()
    ...
    def test_database_restore_procedure(self, test_db_path):
        ...
        restored_account = restored_session.query(DimAccount).filter_by(handle='restore_test').first()
...
class TestDataRecovery:
    ...
    def test_data_integrity_after_recovery(self, test_db_path):
        ...
        session = Session()
        recovered_account = session.query(DimAccount).filter_by(handle='integrity_test').first()
        assert recovered_account is not None
        
        recovered_snapshots = session.query(FactFollowersSnapshot).filter_by(
            account_key=recovered_account.account_key
        ).all()
        assert len(recovered_snapshots) == 1
        assert recovered_snapshots[0].account.account_key == recovered_account.account_key
        session.close()
