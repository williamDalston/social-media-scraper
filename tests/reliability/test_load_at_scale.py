...
class TestScaledLoad:
    ...
    def test_high_concurrency_api_requests(self, client, db_session):
        ...
        accounts = []
        for i in range(50):
            account = DimAccount(platform="X", handle=f"load_test_{i}", org_name="HHS")
        ...
        if total > 0:
            ...
            avg_time = (
                sum(results["times"]) / len(results["times"]) if results["times"] else 0
            )
    ...
    def test_bulk_operations_at_scale(self, db_session):
        ...
        accounts = []
        for i in range(500):
            account = DimAccount(platform="X", handle=f"bulk_scale_{i}", org_name="HHS")
            accounts.append(account)
            db_session.add(account)
        db_session.commit()
        elapsed = time.time() - start
...
class TestSustainedLoad:
    ...
    def test_memory_stability_under_load(self, db_session):
        ...
        for i in range(100):
            account = DimAccount(
                platform="X", handle=f"memory_load_{i}", org_name="HHS"
            )
            db_session.add(account)
            if i % 10 == 0:
                db_session.commit()
