...
class DimAccountFactory(SQLAlchemyModelFactory):
    ...
    platform = Faker(
        "random_element",
        elements=["X", "Instagram", "Facebook", "YouTube", "LinkedIn", "Truth Social"],
    )
    handle = Faker("user_name")
    org_name = Faker("random_element", elements=["HHS", "NIH", "CDC", "FDA", "CMS"])
    account_display_name = LazyAttribute(
        lambda obj: f"{obj.org_name} on {obj.platform}"
    )
    account_url = LazyAttribute(
        lambda obj: f"https://{obj.platform.lower()}.com/{obj.handle}"
    )
    is_core_account = LazyAttribute(lambda obj: obj.org_name == "HHS")
    is_leader_account = False
    requires_preclearance = False
    sensitivity_level = Faker(
        "random_element", elements=["Low", "Medium", "High", None]
    )
...
class FactFollowersSnapshotFactory(SQLAlchemyModelFactory):
    ...
    likes_count = LazyAttribute(
        lambda obj: random.randint(50, obj.followers_count // 10)
    )
    comments_count = LazyAttribute(
        lambda obj: random.randint(5, obj.followers_count // 100)
    )
    shares_count = LazyAttribute(
        lambda obj: random.randint(10, obj.followers_count // 50)
    )
    engagements_total = LazyAttribute(
        lambda obj: (obj.likes_count or 0)
        + (obj.comments_count or 0)
        + (obj.shares_count or 0)
    )
...
class FactSocialPostFactory(SQLAlchemyModelFactory):
    ...
    post_id = Faker("uuid4")
    post_url = LazyAttribute(
        lambda obj: f"https://{obj.platform.lower()}.com/{obj.account.handle}/status/{obj.post_id}"
    )
...
def create_account_with_snapshots(session, num_snapshots=30, **account_kwargs):
    ...
    for i in range(num_snapshots):
        ...
        snapshot = FactFollowersSnapshotFactory(
            account=account, snapshot_date=snapshot_date
        )
