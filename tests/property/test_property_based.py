...
class TestExtractHandleProperty:
    ...
    @given(
        ...
        platform=st.sampled_from(['X', 'Facebook', 'Instagram', 'LinkedIn', 'YouTube', 'Truth Social'])
    )
    ...
    @given(
        handle=st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=('L', 'N', '_'))),
...
class TestAccountModelProperties:
    ...
    @settings(max_examples=50)
    def test_account_creation_with_various_inputs(self, db_session, platform, handle, org_name):
...
class TestSnapshotProperties:
    ...
    @settings(max_examples=50)
    def test_snapshot_creation_with_various_values(self, db_session, sample_account, followers, engagement, snapshot_date):
    ...
    def test_engagement_total_calculation(self, db_session, sample_account, likes, comments, shares):
...
class TestDataConsistencyProperties:
    ...
    def test_sequential_snapshots_consistency(self, db_session, sample_account, num_snapshots, base_followers):
...
class TestURLParsingProperties:
    ...
    @given(
        ...
        path=st.text(min_size=1, max_size=100, alphabet=st.characters(whitelist_categories=('L', 'N', '/', '_', '-')))
    )
    @settings(max_examples=30)
