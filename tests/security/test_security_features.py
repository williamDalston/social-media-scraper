...
class TestRateLimiting:
    ...
    def test_api_rate_limit(self, client, auth_token):
        ...
        # Make many requests
        for i in range(101):
            response = client.get('/api/summary', headers={
                'Authorization': f'Bearer {auth_token}'
            })
...
class TestInputValidation:
    ...
    def test_sql_injection_protection(self, client, auth_token):
        ...
        response = client.get('/api/history/x/test\'; DROP TABLE users;--', headers={
            'Authorization': f'Bearer {auth_token}'
        })
    ...
    def test_xss_protection(self, client, auth_token):
        """Test XSS protection."""
        # Attempt XSS
        response = client.get('/api/history/x/<script>alert("xss")</script>', headers={
            'Authorization': f'Bearer {auth_token}'
        })
...
class TestAuditLogging:
    ...
    def test_security_events_logged(self, client, auth_token):
        ...
        response = client.get('/api/auth/audit-logs', headers={
            'Authorization': f'Bearer {auth_token}'
        })
...
class TestCompliance:
    ...
    def test_gdpr_data_export(self, client, auth_token):
        ...
        response = client.get('/api/auth/gdpr/export', headers={
            'Authorization': f'Bearer {auth_token}'
        })
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_gdpr_data_deletion(self, client, auth_token):
        ...
        response = client.post('/api/auth/gdpr/delete', 
            headers={'Authorization': f'Bearer {auth_token}'},
            json={'anonymize': True}
        )
        assert response.status_code == 200
