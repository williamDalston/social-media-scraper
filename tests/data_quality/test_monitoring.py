...
class TestDataQualityMetrics:
    ...
    def test_calculate_quality_metrics(self, db_session):
        ...
        completeness_rate = (
            (complete_snapshots / total_snapshots * 100) if total_snapshots > 0 else 0
        )
...
class TestQualityReporting:
    ...
    def test_generate_quality_report(self, db_session):
        ...
        for i in range(10):
            account = DimAccount(
                platform="X", handle=f"quality_report_{i}", org_name="HHS"
            )
            db_session.add(account)
        ...
        report = {
            ...
            "accounts_with_snapshots": db_session.query(DimAccount.account_key)
            .join(FactFollowersSnapshot)
            .distinct()
            .count(),
            "recent_snapshots": db_session.query(FactFollowersSnapshot)
