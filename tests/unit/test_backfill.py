...
class TestBackfillHistory:
    ...
    def test_backfill_history_skips_existing_snapshots(self, test_db_path):
        ...
        existing = (
            session.query(FactFollowersSnapshot)
            .filter_by(account_key=account.account_key, snapshot_date=existing_date)
            .first()
        )
    ...
    def test_backfill_history_handles_hhs_accounts(self, test_db_path):
        ...
        hhs_account = DimAccount(
            platform="X", handle="hhs", org_name="HHS", account_url="https://x.com/hhs"
        )
        non_hhs_account = DimAccount(
            platform="X", handle="nih", org_name="NIH", account_url="https://x.com/nih"
        )
        ...
        hhs_snapshots = (
            session.query(FactFollowersSnapshot)
            .filter_by(account_key=hhs_account.account_key)
            .all()
        )
        nih_snapshots = (
            session.query(FactFollowersSnapshot)
            .filter_by(account_key=non_hhs_account.account_key)
            .all()
        )
