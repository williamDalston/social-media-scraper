...
class TestDimAccount:
    ...
    def test_account_repr(self, db_session):
        ...
        account = DimAccount(platform="Instagram", handle="test", org_name="NIH")
        db_session.add(account)
        db_session.commit()
    ...
    def test_account_optional_fields(self, db_session):
        ...
        account = DimAccount(platform="X", handle="minimal")
        db_session.add(account)
        db_session.commit()
    ...
    def test_account_default_values(self, db_session):
        ...
        account = DimAccount(platform="X", handle="test")
        db_session.add(account)
        db_session.commit()
...
class TestFactSocialPost:
    ...
    def test_post_relationship(self, db_session, sample_account):
        ...
        post = FactSocialPost(
            account_key=sample_account.account_key, platform="X", post_id="12345"
        )
        db_session.add(post)
        db_session.commit()
...
class TestModelRelationships:
    ...
    def test_account_has_multiple_posts(self, db_session, sample_account):
        ...
        post1 = FactSocialPost(
            account_key=sample_account.account_key, platform="X", post_id="1"
        )
        post2 = FactSocialPost(
            account_key=sample_account.account_key, platform="X", post_id="2"
        )
        ...
        posts = (
            db_session.query(FactSocialPost)
            .filter_by(account_key=sample_account.account_key)
            .all()
        )
