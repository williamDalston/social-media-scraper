...
def login():
    ...
    if user:
        from security.account_takeover import detect_account_takeover_risk, log_account_takeover_attempt, require_additional_verification
...
def refresh_token():
    ...
    try:
        ...
        if old_version < user.token_version:
            ...
            session.close()
            return jsonify({'error': 'Token has been rotated. Please login again.'}), 401
