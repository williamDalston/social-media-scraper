...
# Platform whitelist
ALLOWED_PLATFORMS = ['x', 'twitter', 'instagram', 'facebook', 'linkedin', 'youtube', 'truth_social', 'tiktok']
...
def validate_csv_file(file_content: bytes, max_size_mb: int = 10) -> tuple[bool, str, list]:
    """
    ...
    try:
        ...
        for i, row in enumerate(csv_reader, start=2):  # Start at 2 (header is row 1)
            ...
            if not all(col in row for col in required_columns):
                return False, f"Row {i}: Missing required columns. Required: {', '.join(required_columns)}", []
            ...
            if platform not in ALLOWED_PLATFORMS:
                return False, f"Row {i}: Invalid platform '{platform}'. Allowed: {', '.join(ALLOWED_PLATFORMS)}", []
