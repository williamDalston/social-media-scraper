...
performance_sla_bp = Blueprint('performance_slas', __name__, url_prefix='/api/performance/slas')
...
def get_performance_sla_status():
    ...
    try:
        ...
        metrics = {
            ...
            'database_query_time': query_stats.get('p95_time', 0) * 1000,  # Convert to ms
            'cache_hit_rate': cache_metrics.get('cache_hit_rate', 0) * 100,  # Convert to percent
            'scraper_success_rate': cache_metrics.get('scraper_success_rate', 0) * 100,  # Convert to percent
        }
        ...
        compliant_slas = sum(1 for s in sla_status.values() if s.get('meets_sla', False))
