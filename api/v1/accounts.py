...
upload_response_model = ns.model(
    "UploadResponse",
    {
...
account_model = ns.model(
    "Account",
    {
        ...
        "is_active": fields.Boolean(
            description="Whether account is active for scraping"
        ),
    },
)
...
class Upload(Resource):
    ...
    def post(self):
        ...
        file = (
            ...
            if hasattr(request, "validated_file")
            ...
            try:
                for row_num, row in enumerate(
                    ...
                    if platform not in valid_platforms:
                        errors.append(
                            f'Row {row_num}: Invalid platform "{platform}". Valid platforms: {", ".join(valid_platforms)}"'
                        )
                        continue
                ...
                if errors:
                    message += (
                        f'. {len(errors)} rows had errors: {"; ".join(errors[:5])}'
                    )
...
class AccountList(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            # Apply pagination
            accounts = (
                query.order_by(DimAccount.platform, DimAccount.handle)
...
class AddAccount(Resource):
    ...
    add_account_model = ns.model(
        "AddAccount",
        {
            "platform": fields.String(
                required=True, description="Social media platform"
            ),
    ...
    def post(self):
        ...
        try:
            ...
            account = DimAccount(
                ...
                account_display_name=data.get(
                    "account_display_name", f"{handle} on {platform}"
                ),
...
class ToggleActive(Resource):
    ...
    def post(self, account_key):
        ...
        try:
            ...
            account.is_active = not (
                account.is_active if account.is_active is not None else True
            )
            session.commit()
