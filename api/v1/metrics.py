...
from api.schemas import (
    SummaryItemSchema, HistoryDataSchema, GridRowSchema,
...
summary_item_model = ns.model('SummaryItem', {
...
history_data_model = ns.model('HistoryData', {
    'dates': fields.List(fields.Date(), description='Snapshot dates'),
    'followers': fields.List(fields.Integer(), description='Follower counts over time'),
    'engagement': fields.List(fields.Integer(), description='Engagement totals over time')
})

grid_row_model = ns.model('GridRow', {
...
class Summary(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            latest_date = session.query(FactFollowersSnapshot.snapshot_date)\
            ...
            results = session.query(DimAccount, FactFollowersSnapshot)\
                .join(FactFollowersSnapshot)\
                .filter(FactFollowersSnapshot.snapshot_date == latest_date).all()
...
class History(Resource):
    ...
    @ns.doc('get_history')
    @ns.param('platform', 'Social media platform', _in='path', required=True,
    ...
    def get(self, platform, handle):
        ...
        try:
            ...
            if not account:
                raise NotFoundError(resource_type='Account', 
                                  resource_id=f'{platform}/{handle}')
            
            history = session.query(FactFollowersSnapshot)\
...
class Grid(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            ).join(FactFollowersSnapshot).all()
...
class Download(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            writer.writerow([
                'Platform', 'Handle', 'Organization', 'Date', 'Followers',
                'Engagement Total', 'Posts', 'Likes', 'Comments', 'Shares'
            ])
