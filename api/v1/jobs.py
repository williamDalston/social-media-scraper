...
scraper_request_model = ns.model(
    "ScraperRequest",
    {
        "mode": fields.String(
...
scraper_selected_request_model = ns.model(
    "ScraperSelectedRequest",
    {
        "mode": fields.String(
            description="Scraper mode",
            enum=["simulated", "real"],
...
scraper_response_model = ns.model(
    "ScraperResponse",
    {
    ...
    def post(self):
        ...
        if mode != "real":
            raise BadRequestError(
                "Only real mode is supported. Simulations are disabled."
            )
...
class RunScraperSelected(Resource):
    ...
    def post(self):
        ...
        try:
            db_path = get_db_path()
            # Trigger async task
            task = scrape_selected_accounts.delay(
                account_keys=account_keys, mode=mode, db_path=db_path
            )
