...
performance_analytics_model = ns.model(
...
class JobFailures(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            failed_jobs = (
                session.query(Job)
                .filter(Job.status == "failed", Job.created_at >= cutoff_date)
            ...
            for job in failed_jobs:
                ...
                error_type = (
                    error_msg.split(":")[0].split()[0] if error_msg else "Unknown"
                )
                if error_type not in error_groups:
                    error_groups[error_type] = {"count": 0, "jobs": [], "latest": None}
                ...
                if (
                    not error_groups[error_type]["latest"]
                    or job.created_at > error_groups[error_type]["latest"]
                ):
                    error_groups[error_type]["latest"] = (
