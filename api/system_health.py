...
from config.error_detection import (
    ErrorDetector, get_error_summary, get_fix_suggestion,
...
def get_system_status():
    ...
    try:
        ...
        system_passed, system_results = system_validator.validate_all(skip_optional=False)
        system_summary = system_validator.get_summary()
        
        pipeline_passed, pipeline_summary = validate_pipeline()
        
        db_path = os.getenv('DATABASE_PATH', 'social_media.db')
        health_results = run_health_checks(db_path=db_path, include_optional=True, use_cache=False)
...
def get_diagnostics():
    ...
    try:
        ...
        try:
            db_path = os.getenv('DATABASE_PATH', 'social_media.db')
            health_results = run_health_checks(db_path=db_path, include_optional=False, use_cache=False)
