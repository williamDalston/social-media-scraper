...
def calculate_optimal_schedule_time(job_type, historical_data=None):
    ...
    try:
        ...
        if not completed_jobs:
            ...
            optimal_hour = min(hour_performance.items(), key=lambda x: x[1]['avg_duration'])[0]
...
def should_retry_job(job, error_type=None):
    ...
    permanent_errors = [
        'not found', '404', '401', '403', 'invalid', 'malformed',
...
def optimize_job_distribution():
    ...
    try:
        ...
        return {
            ...
            'recommendation': 'Rebalance jobs' if imbalance_percent > 20 else 'Distribution is balanced'
        }
...
def get_resource_usage_optimization():
    ...
    try:
        ...
        long_running = [j for j in recent_jobs if j.duration_seconds and j.duration_seconds > 3600]
        if long_running:
