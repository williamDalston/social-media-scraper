...
def create_dependent_job(parent_job_id, job_type, task_func, **task_kwargs):
    ...
    try:
        ...
        job.result = json.dumps(
            {"task_name": task_func.name, "task_kwargs": task_kwargs}
        )
        session.commit()
...
def check_and_start_dependent_jobs(parent_job_id):
    ...
    try:
        ...
        dependent_jobs = (
            session.query(Job)
            .filter_by(depends_on_job_id=parent_job_id, status="pending", paused="true")
            .all()
        )
                ...
                try:
                    ...
                    if task_name:
                        ...
                        if task_func:
                            ...
                            session.commit()
                            logger.info(
                                f"Started dependent job {job.id} as {result.id}"
                            )
                        else:
                            logger.error(
                                f"Task {task_name} not found for dependent job {job.id}"
                            )
                    else:
