...
def scrape_account(self, account_key, mode="real", db_path=None):
    ...
    try:
        ...
        if not isinstance(account_key, (int, str)):
            raise ValueError(
                f"account_key must be int or str, got {type(account_key).__name__}"
            )
        ...
        try:
            account = (
                session.query(DimAccount).filter_by(account_key=account_key).first()
            )
            if not account:
                raise ValueError(f"Account with key {account_key} not found")
        except Exception as e:
            logger.error(
                f"Error querying account: {e}", extra={"account_key": account_key}
            )
            raise
        ...
        try:
            self.update_state(
                state="PROGRESS",
                meta={"progress": 25, "message": f"Scraping {account.handle}..."},
            )
            update_job_progress(
                job_id, 25, "running", {"message": f"Scraping {account.handle}..."}
            )
        except Exception as e:
        ...
        logger.info(
            f"Starting scrape for account {account.handle} (key: {account_key})"
        )
        try:
            ...
            logger.error(
                f"Error getting scraper: {e}",
                extra={"mode": mode, "account_key": account_key},
            )
            raise
        ...
        if not data:
            error_msg = (
                f"Failed to scrape account {account.handle} - scraper returned no data"
            )
            logger.error(
                error_msg,
                extra={"account_key": account_key, "platform": account.platform},
            )
        ...
        if not isinstance(data, dict):
            raise ValueError(
                f"Scraped data must be a dictionary, got {type(data).__name__}"
            )

        # Update progress
        self.update_state(
            state="PROGRESS", meta={"progress": 75, "message": "Saving results..."}
        )
        ...
        existing = (
            ...
            .filter_by(account_key=account_key, snapshot_date=today)
            .first()
        )
        ...
        if existing:
            ...
            try:
                ...
                existing.subscribers_count = safe_int(
                    data.get("subscribers_count"), 0
                )  # For YouTube
                existing.video_views = safe_int(
                    data.get("views_count"), 0
                )  # For YouTube
                existing.videos_count = safe_int(
                    data.get("videos_count"), 0
                )  # For YouTube
                existing.engagements_total = (
                    existing.likes_count
                    + existing.comments_count
                    + existing.shares_count
                )
        ...
        if self.request.retries < self.max_retries:
            ...
            if should_retry:
                ...
                logger.info(
                    f"Retrying scrape_account task (attempt {self.request.retries + 1}/{self.max_retries}) in {retry_countdown}s - {reason}"
                )
...
def scrape_all_accounts(self, mode="real", db_path=None):
    ...
    try:
        ...
        self.update_state(
            ...
            meta={
                "progress": 10,
                "message": f"Starting scrape for {total_accounts} accounts...",
            },
        )
        update_job_progress(
            job_id,
        ...
        self.update_state(
            state="PROGRESS", meta={"progress": 20, "message": "Running scraper..."}
        )
        ...
        self.update_state(
            state="PROGRESS", meta={"progress": 90, "message": "Finalizing..."}
        )
...
def scrape_platform(self, platform, mode="real", db_path=None):
    ...
    try:
        ...
        self.update_state(
            ...
            meta={
                "progress": 10,
                "message": f"Starting scrape for {total_accounts} {platform} accounts...",
            },
        )
        update_job_progress(
            job_id,
        ...
        for i, account in enumerate(accounts):
            ...
            self.update_state(
                state="PROGRESS",
                meta={"progress": progress, "message": f"Scraping {account.handle}..."},
            )
            update_job_progress(
                job_id,
                progress,
                "running",
                {"message": f"Scraping {account.handle}..."},
            )
...
def scrape_selected_accounts(self, account_keys, mode="real", db_path=None):
    ...
    try:
        ...
        if total_accounts == 0:
            raise ValueError(
                f"No accounts found for the provided account keys: {account_keys}"
            )
        ...
        logger.info(
            f"Starting scrape_selected_accounts job for {total_accounts} accounts"
        )
        ...
        self.update_state(
            ...
            meta={
                "progress": 10,
                "message": f"Starting scrape for {total_accounts} selected accounts...",
            },
        )
        update_job_progress(
            job_id,
...
def backfill_account(self, account_key, days=365, db_path=None):
    ...
    try:
        ...
        self.update_state(
            ...
            meta={
                "progress": 10,
                "message": f"Starting backfill for {account.handle}...",
            },
        )
        update_job_progress(
            job_id,
