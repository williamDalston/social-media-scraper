...
def get_queue_depth(queue_name="scraping"):
    ...
    try:
        ...
        try:
            return (
                session.query(Job)
                .filter(Job.status == "pending", Job.paused == "false")
                .count()
            )
        finally:
            session.close()
...
def check_job_dependencies(job_id):
    ...
    try:
        ...
        if dependency.status == "completed":
            ...
            return False, [
                f"Dependency job {job.depends_on_job_id} is {dependency.status}"
            ]
    finally:
        session.close()
...
def calculate_job_sla_status(job):
    ...
    return {
        ...
        "sla_violation_percent": (
            (actual_seconds - job.sla_seconds) / job.sla_seconds * 100
        )
        if actual_seconds > job.sla_seconds
        else 0,
    }
    ...
    try:
        ...
        for job_type in set(j.job_type for j in jobs):
            ...
            by_type[job_type] = {
                ...
                "success_rate": (
                    sum(1 for j in type_jobs if j.status == "completed")
                    / len(type_jobs)
                    * 100
                )
                if type_jobs
        ...
        return {
            ...
            "sla_compliance_rate": (sla_met / sla_total * 100)
            if sla_total > 0
            else None,
            "by_job_type": by_type,
        }
