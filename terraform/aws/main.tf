  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "python -c \"import urllib.request; urllib.request.urlopen('http://localhost:5000/health', timeout=5)\""]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 40
...
resource "aws_elasticache_replication_group" "main" {
  ...
  description                = "Redis for Social Media Scraper"
...
resource "aws_secretsmanager_secret" "app_secrets" {
  ...
  description = "Secrets for Social Media Scraper application"
