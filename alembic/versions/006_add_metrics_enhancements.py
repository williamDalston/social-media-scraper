...
def upgrade() -> None:
    ...
    op.add_column('dim_account', sa.Column('account_created_date', sa.Date(), nullable=True))
    op.add_column('dim_account', sa.Column('account_category', sa.String(), nullable=True))
    ...
    op.add_column('dim_account', sa.Column('profile_image_url', sa.String(), nullable=True))
    
    # Add calculated metrics to fact_followers_snapshot
    op.add_column('fact_followers_snapshot', sa.Column('engagement_rate', sa.Float(), nullable=True))
    op.add_column('fact_followers_snapshot', sa.Column('follower_growth_rate', sa.Float(), nullable=True))
    op.add_column('fact_followers_snapshot', sa.Column('follower_growth_absolute', sa.Integer(), nullable=True))
    op.add_column('fact_followers_snapshot', sa.Column('posts_per_day', sa.Float(), nullable=True))
    
    # Add YouTube-specific video metrics
    op.add_column('fact_followers_snapshot', sa.Column('total_video_views', sa.Integer(), nullable=True))
    op.add_column('fact_followers_snapshot', sa.Column('average_views_per_video', sa.Float(), nullable=True))
    
    # Create indexes for new fields
    op.create_index('ix_dim_account_created_date', 'dim_account', ['account_created_date'])
    op.create_index('ix_fact_snapshot_engagement_rate', 'fact_followers_snapshot', ['engagement_rate'])
    op.create_index('ix_fact_snapshot_growth_rate', 'fact_followers_snapshot', ['follower_growth_rate'])


def downgrade() -> None:
    # Drop indexes
    op.drop_index('ix_fact_snapshot_growth_rate', table_name='fact_followers_snapshot')
    op.drop_index('ix_fact_snapshot_engagement_rate', table_name='fact_followers_snapshot')
