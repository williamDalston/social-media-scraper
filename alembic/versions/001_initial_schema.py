...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "requires_preclearance", sa.Boolean(), nullable=True, server_default="0"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "has_sensitive_topic_flag", sa.Boolean(), nullable=True, server_default="0"
        ),
    ...
    op.create_index(
        "ix_fact_followers_snapshot_account_key",
        "fact_followers_snapshot",
        ["account_key"],
    )
    op.create_index(
        "ix_fact_followers_snapshot_snapshot_date",
        "fact_followers_snapshot",
        ["snapshot_date"],
    )
    op.create_index(
        "ix_fact_social_post_account_key", "fact_social_post", ["account_key"]
    )
    op.create_index("ix_fact_social_post_platform", "fact_social_post", ["platform"])
    op.create_index(
        "ix_fact_social_post_post_datetime_utc",
        "fact_social_post",
        ["post_datetime_utc"],
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_fact_social_post_post_datetime_utc", table_name="fact_social_post"
    )
    ...
    op.drop_index(
        "ix_fact_followers_snapshot_snapshot_date", table_name="fact_followers_snapshot"
    )
    op.drop_index(
        "ix_fact_followers_snapshot_account_key", table_name="fact_followers_snapshot"
    )
