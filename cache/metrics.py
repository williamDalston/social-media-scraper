    ...
    def get_api_stats(self, endpoint: Optional[str] = None) -> Dict:
        ...
        with self._lock:
            if endpoint:
                ...
                return {ep: self.get_api_stats(ep) for ep in self.api_counts.keys()}
    ...
    def get_cache_stats(self) -> Dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "miss_rate": (total_misses / total_requests * 100)
                if total_requests > 0
                else 0,
                "by_key": {
                    key: {
                        ...
                        "hit_rate": (
                            self.cache_hits[key]
                            / (self.cache_hits[key] + self.cache_misses.get(key, 0))
                            * 100
                            if (self.cache_hits[key] + self.cache_misses.get(key, 0))
                            > 0
                            else 0
                        ),
                    }
        ...
        with self._lock:
            ...
            return {
                ...
                "success_rate": (
                    ...
                    / (self.scraper_success + self.scraper_errors)
                    * 100
                    if (self.scraper_success + self.scraper_errors) > 0
                    else 0
                ),
                "avg_time": sum(times) / len(times) if times else 0,
                "min_time": min(times) if times else 0,
