...
class CacheWarmer:
    ...
    def register_warming_task(
        self, task: Callable, name: str, interval_minutes: int = 5
    ):
        """Register a cache warming task."""
        self.warming_tasks.append(
            {"task": task, "name": name, "interval": interval_minutes, "last_run": None}
        )
...
def register_default_warming_tasks():
    ...
    warmer.register_warming_task(
        warm_account_list_cache, "account_list", interval_minutes=10
    )
