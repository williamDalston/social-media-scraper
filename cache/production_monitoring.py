...
class CachePerformanceMonitor:
    ...
    def __init__(self, max_history: int = 10000):
        ...
        self._key_stats = defaultdict(lambda: {'hits': 0, 'misses': 0, 'sets': 0, 'deletes': 0})
    ...
    def get_overall_stats(self) -> Dict:
        ...
        with self._lock:
            ...
            l1_hit_rate = (
                (self._l1_stats['hits'] / total_l1_ops * 100)
                if total_l1_ops > 0 else 0
            )
            l2_hit_rate = (
                (self._l2_stats['hits'] / total_l2_ops * 100)
                if total_l2_ops > 0 else 0
            )
            ...
            overall_hit_rate = (
                (total_hits / total_ops * 100) if total_ops > 0 else 0
            )
            ...
            if timing_list:
                ...
                p95_duration = sorted_durations[p95_idx] if p95_idx < len(sorted_durations) else 0
                p99_duration = sorted_durations[p99_idx] if p99_idx < len(sorted_durations) else 0
            else:
                p95_duration = 0
                p99_duration = 0
            ...
            return {
                ...
                'performance': {
                    'avg_duration': sum(d['duration'] for d in timing_list) / len(timing_list) if timing_list else 0,
                    'p95_duration': p95_duration,
                    'p99_duration': p99_duration,
                    'min_duration': min(d['duration'] for d in timing_list) if timing_list else 0,
                    'max_duration': max(d['duration'] for d in timing_list) if timing_list else 0
                },
