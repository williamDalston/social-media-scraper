...
class CacheWarmer:
    ...
    def warm_top_accounts(self, accounts: List[dict], get_history_func: Callable, limit: int = 10):
        """Warm cache for top N accounts by followers."""
        try:
            logger.info(f"Warming cache for top {limit} accounts...")
            # Sort by followers (descending)
            sorted_accounts = sorted(accounts, key=lambda x: x.get('followers', 0), reverse=True)
